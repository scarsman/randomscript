...
TEMPFILE=$(mktemp)
PFLOGSUMM=/home/pol/scripts/pflogsumm
PYGTAIL=/home/pol/scripts/pygtail.py
ZABBIXUSER=pol
...
if [ ! -w "${PFSTATSFILE}" ]; then
        touch "${PFSTATSFILE}" && chown ${ZABBIXUSER}:${ZABBIXUSER}"${PFSTATSFILE}" > /dev/null 2>&1

        if [ ! $? -eq 0 ]; then
                result_text="ERROR: wrong exit code returned while creating file ${PFSTATSFILE} and setting its owner to ${ZABBIXUSER}:${ZABBIXUSER}"
                result_code="1"
